...
public interface InventoryLogRepository extends JpaRepository<InventoryLog, Long> {
    ...
    // 품목+창고별 로그 조회
    Page<InventoryLog> findByItemIdAndWarehouseIdOrderByLogDatetimeDesc(
        Long itemId, Long warehouseId, Pageable pageable);
    ...
    @Query("SELECT il FROM InventoryLog il " +
           "WHERE il.logDatetime BETWEEN :startDate AND :endDate " +
    ...
    // 마지막 입고일 조회
    @Query("SELECT MAX(il.logDatetime) FROM InventoryLog il " +
           "WHERE il.item.id = :itemId " +
           "AND il.warehouse.id = :warehouseId " +
           "AND il.logType = 'IN'")
    Optional<LocalDateTime> findLastInDate(@Param("itemId") Long itemId, 
    ...
    @Query("SELECT MAX(il.logDatetime) FROM InventoryLog il " +
           ...
           "AND il.warehouse.id = :warehouseId " +
           "AND il.logType = 'OUT'")
    Optional<LocalDateTime> findLastOutDate(@Param("itemId") Long itemId, 
                                           @Param("warehouseId") Long warehouseId);
    
    // 기간 내 입고 총량
    @Query("SELECT SUM(il.quantity) FROM InventoryLog il " +
           "WHERE il.item.id = :itemId " +
           "AND il.warehouse.id = :warehouseId " +
           "AND il.logType = 'IN' " +
           "AND il.logDatetime BETWEEN :startDate AND :endDate")
    Optional<Integer> sumInQuantityByPeriod(@Param("itemId") Long itemId,
                                           @Param("warehouseId") Long warehouseId,
    ...
    @Query("SELECT SUM(il.quantity) FROM InventoryLog il " +
           ...
           "AND il.logType = 'OUT' " +
           "AND il.logDatetime BETWEEN :startDate AND :endDate")
    Optional<Integer> sumOutQuantityByPeriod(@Param("itemId") Long itemId,
                                            @Param("warehouseId") Long warehouseId,
                                            @Param("startDate") LocalDateTime startDate,
                                            @Param("endDate") LocalDateTime endDate);
}