...
function goToInboundPage() {
    window.location.href = '/place/receive';
}
    ...
    document.querySelectorAll('.item-row').forEach((row, index) => {
        ...
        if (itemTypeSelect && itemTypeSelect.value && itemNameSelect && itemNameSelect.value) {
            const lineItem = {
                ...
                unitPriceAll: unitPriceAllInput ? 
                              parseFloat(unitPriceAllInput.value.replace(/,/g, '')) || 0 : 0
            };
...
// ========================
// 11) 사업장 유형 선택에 따라 해당 사업장명 옵션만 표시
// ========================
...
// ========================
// 13) item_type 선택 시 해당 타입의 품목 리스트를 불러와 item_name에 채움
// ========================
...
// ========================
// 14) 품목명 선택 시 코드, 단위, 단가 입력 필드 자동 입력
// ========================
...
function calculateTotals() {
    ...
    rows.forEach(row => {
        ...
        const qty      = parseInt(qtyInput?.value) || 0;
        const priceAll = parseFloat(priceAllInput?.value) || 0;
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    // ========================
    // 21) 페이지 로드 시 “itemsParam” 파싱하여 자동으로 발주 등록 모달 띄우고 행 채우기
    // ========================
    const itemsParam = /*[[${itemsParam}]]*/ null;
    if (itemsParam) {
        // 파싱 함수: "RAW001:4470,RAW002:8940" → [{itemCode:"RAW001", quantity:4470}, ...]
        function parseItemsParam(param) {
            return param.split(',')
                        .map(pair => pair.trim())
                        .map(pair => {
        ...
        const parsedList = parseItemsParam(itemsParam);

        // 발주 등록 모달을 연다
        openplaceAddModal();

        // 각 parsedList 항목마다 품목 행 추가 및 값 채워주기
        const container = document.getElementById('itemListContainer');
        container.innerHTML = ''; // 기존 행 초기화
        ...
        parsedList.forEach((item, idx) => {
            ...
            newRow.innerHTML = `
                ...
                <td><input type="text" class="form-control item-code" name="lineItems[${idx}].itemCode" readonly /></td>
                <td><input type="number" class="form-control unit-qty" name="lineItems[${idx}].unitQty" min="1" /></td>
                <td><input type="text" class="form-control item-unit" name="lineItems[${idx}].unit" readonly /></td>
                <td><input type="text" class="form-control unit-price" name="lineItems[${idx}].unitPrice" readonly /></td>
                <td><input type="hidden" name="lineItems[${idx}].orderLineItemId" class="order-line-item-id" value="" /></td>
                <td><button type="button" class="btn btn-danger btn-sm" onclick="removeItemRow(this)">삭제</button></td>
            `;
            ...
            // “품목코드” 칸에는 넘어온 item.itemCode 그대로 채움
